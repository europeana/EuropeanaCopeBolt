      ...
      success: function(data) {
        ...
        if (data.status !== 'error' && data.results[data.type]) {
          ...
          results.forEach(function(e, index) {
            ...
            var title = (e.title)?e.title:(e.name)?e.name:(e.last_name)?e.first_name + ' ' + e.last_name:(e.position)?e.position:'no title';
  ...
  $('div.selectasync-select:not(.ispreloaded) input').each(function() {
    ...
    $('#' + $(selectholder).attr('id')).autocomplete({
      ...
      select: function( event, ui ) {
        ...
        var title = (ui.item.full_item.title)?ui.item.full_item.title:(ui.item.full_item.name)?ui.item.full_item.name:(ui.item.full_item.last_name)?ui.item.full_item.first_name + ' ' + ui.item.full_item.last_name:(ui.full_item.position)?ui.full_item.position:'no title';
        // make sure there is a status
